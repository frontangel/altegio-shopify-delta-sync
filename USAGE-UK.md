...
- Тести: `yarn test` або `npm test` — запускає базові юніт-тести конфігурації, кроків пайплайна та черги (використовується вбудований runner Node.js).
...
- `POST /sync/all` — ставить у чергу всі товари зі складу Altegio для повної синхронізації залишків (під базовою автентифікацією).
...
- Якщо Redis деградує, сервіс логуватиме перехід на дискову чергу (`🚨 Redis degraded...`), що дає DevOps сигнал для моніторингу.
- Події, які не вдалося обробити (невідомий ресурс чи помилка валідації), зберігаються у `store/dead-letter.json` для ручного розслідування/повторної обробки.
